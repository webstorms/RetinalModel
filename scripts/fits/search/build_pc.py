...
def build_pcs(dataset_name, spatial_args, luminance=1, stride=4):
    for pred_ms in [0]:
        for spatial_scale, max_dim in spatial_args:
            ...
            build_output(dataset_name, pred_ms, spatial_scale, max_dim, pad=5, luminance=luminance, stride=stride)


def build_output(dataset_name, pred_ms, spatial_scale, max_dim, pad=5, luminance=1, stride=4):
    ...
    pc_builder = pca.PCBuilder(root, lam=10**-2.5, pred_ms=pred_ms, model=model, train_dataset=train_dataset, test_dataset=test_dataset, n_pca=500, pad_t_len=29, stride=stride, repeats=8)
